...
class Artikel extends BaseController
{
    ...
    public function admin_index()
    {
        ...
        log_message('info', 'admin_index started with q=' . $q . ', kategori_id=' . $kategori_id);

        // Bangun query dengan optimasi
        $builder = $model
            ->select('artikel.*, kategori.nama_kategori')
            ->join('kategori', 'kategori.id_kategori = artikel.id_kategori', 'left')
            ->orderBy('artikel.id', 'DESC'); // Tambahkan ordering untuk konsistensi
        ...
        // Try simple approach first to avoid timeout
        try {
            log_message('info', 'Starting pagination query');
            $artikel = $builder->paginate(10);
            $pager = $model->pager;
            log_message('info', 'Pagination completed successfully');
        } catch (\Exception $e) {
            // Fallback to simple findAll if pagination fails
            log_message('error', 'Pagination failed: ' . $e->getMessage());
            $artikel = $builder->findAll(10);
            $pager = null;
        }
